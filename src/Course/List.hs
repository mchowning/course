...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight foldHelper (Full Nil)
  where
    foldHelper :: Optional a -> Optional (List a) -> Optional (List a)
    foldHelper Empty _ = Empty
    foldHelper _ Empty = Empty
    foldHelper (Full a) (Full ls) = Full (a :. ls)
-- seqOptional = foldRight (twiceOptional (:.)) (Full Nil)
-- todo -- worth looking at this more


-- seqOptional ls
--   | hasEmpty ls = Empty
--   | otherwise   = Full (map extract ls)
--     where
--       hasEmpty :: List (Optional a) -> Bool
--       hasEmpty ls' = 0 /= (length . filter (not . isFull) $ ls')
--
--       isFull :: Optional a -> Bool
--       isFull (Full _) = True
--       isFull _        = False
--
--       extract :: Optional a -> a
--       extract (Full a) = a
--       extract _        = undefined
...
produce = produce' Nil
  where
    produce' :: List a -> (a -> a) -> a -> List a
    produce' ls f x = x :. produce' ls f (f x)
--     produce' f' x'  ls = x' :. produce' f' (f' x') ls
