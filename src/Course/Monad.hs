...
(<*>) ::
  ...
  -> f a
  -> f b
fun <*> fa = (\fun' -> return . fun' =<< fa) =<< fun
-- fun <*> fa = do fun' <- fun
--                 a <- fa
--                 return $ fun' a
-- the following implementation seems to work, but it gives a different order
-- fun <*> fa = (\a -> (\f -> return $ f a) =<< fun) =<< fa
...
(<=<) ::
  ...
  -> f c
fbc <=< fab = (=<<) fbc . fab
-- fbc <=< fab = \a -> do b <- fab a
--                        fbc b
-- fbc <=< fab = \a -> fab a >>= \b -> fbc b
